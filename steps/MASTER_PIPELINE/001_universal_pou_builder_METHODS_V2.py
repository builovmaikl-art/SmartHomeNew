...
def main() -> int:
    ...
    if old_pous_window:
        old_pou_names = '|'.join(old_pous_window[2])
        log(f'OLD_POU_WINDOW_NAMES={old_pou_names}')
    ...
    old_pous_tree_names = '|'.join(old_pous_tree)
    old_prg_tree_names = '|'.join(old_prg_tree)
    log(f'OLD_POUS_TREE_NAMES={old_pous_tree_names}')
    log(f'OLD_PRG_TREE_NAMES={old_prg_tree_names}')

    OUTPUT_XML.write_text(xml_text, encoding='utf-8')
    verify_text = OUTPUT_XML.read_text(encoding='utf-8', errors='ignore')
    pou_count_in_output = len(re.findall('<pou name="', verify_text))
    method_count_in_output = len(re.findall('<Method name="', verify_text))
    objectid_count_in_output = len(re.findall('<ObjectId>', verify_text))
