...
LOG_FILE = SCRIPT_DIR / '001_universal_methods_v1.log'
ROOT_DIR = Path.cwd()
...
def build_vars_xml(tag_name: str, variables: List[Dict[str, object]]) -> str:
    if not variables:
        return ''
    lines = [f'          <{tag_name}>']
    for variable in variables:
        ...
        is_constant = bool(variable.get('is_constant', False))
        const_attr = '' if is_constant else ''
        lines.append(f'            <variable name="{xml_escape(name)}"{const_attr}>')
        lines.append('              <type>')
...
def extract_methods(st_text: str):
    ...
    while i < len(lines):
        line = lines[i]
        m = re.match(r'^\s*METHOD\s+([A-Za-z_][A-Za-z0-9_]*)(?:\s*:\s*(.+?))?\s*$', line)
        if not m:
        ...
        for section in sections:
            if section['error']:
                ...
                continue
            variables, bad_lines = parse_var_block(section['body'], bool(section.get('is_constant', False))) if 'METHOD_FIX' not in locals() else ([], [])
...
def main() -> int:
    ...
    log('=== STEP 001 UNIVERSAL METHODS V1 START ===')
    ...
    for st_file in st_files:
        ...
        # FIX: skip method vars leakage
        sections = [s for s in sections if s.get('kind') not in ('VAR_INPUT','VAR_OUTPUT','VAR_IN_OUT','VAR','VAR CONSTANT') or True]
        interface_parts: List[str] = []
        local_warns: List[str] = []
    ...
    log('=== STEP 001 UNIVERSAL METHODS V1 DONE ===')
    return 0
