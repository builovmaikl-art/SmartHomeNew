...
new_block = """// === TREND BUFFER UPDATE (SHARED INDEX) ===
GVL_Trend.G_Trend_Buffer_Index := GVL_Trend.G_Trend_Buffer_Index + 1;
IF GVL_Trend.G_Trend_Buffer_Index > 50 THEN
    GVL_Trend.G_Trend_Buffer_Index := 1;
END_IF;
...
if old_block not in text:
    raise SystemExit("Primary trend buffer block not found")

text = text.replace(old_block, new_block, 1)

old_ch2 = """// === TREND CHANNEL 2 (FLOOR TEMP) ===
...
new_ch2 = """// === TREND CHANNEL 2 (FLOOR TEMP, SHARED INDEX) ===
IF GVL_Trend.G_Trend_Channel2_Enabled THEN
...
if old_ch2 not in text:
    raise SystemExit("Channel 2 trend block not found")

text = text.replace(old_ch2, new_ch2, 1)

path.write_text(text, encoding="utf-8")
print("OK: synchronized trend channels on shared index")
