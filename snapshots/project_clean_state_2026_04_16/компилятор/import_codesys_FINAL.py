...
def detect_base_xml(compiler_dir: Path) -> Path:
    preferred = compiler_dir / 'Система умного дома.xml'
    if preferred.exists():
        return preferred
    xml_files = sorted(compiler_dir.glob('*.xml'))
    if len(xml_files) == 1:
        return xml_files[0]
    if not xml_files:
        raise FileNotFoundError(f'XML not found рядом со скриптом: {compiler_dir}')
    raise RuntimeError('Найдено несколько XML рядом со скриптом. Укажи --xml явно.')


def resolve_path(base_dir: Path, raw: str | None, default_name: str | None = None) -> Path:
    if raw:
        p = Path(raw)
...
def script_layout(script_file: Path, xml_arg: str | None, out_arg: str | None, logs_arg: str | None):
    compiler_dir = script_file.resolve().parent
    project_root = find_project_root(compiler_dir)
    base_xml = resolve_path(compiler_dir, xml_arg, None) if xml_arg else detect_base_xml(compiler_dir)
    out_dir = resolve_path(compiler_dir, out_arg, 'out')
    logs_dir = resolve_path(compiler_dir, logs_arg, 'logs')
    out_dir.mkdir(parents=True, exist_ok=True)
    logs_dir.mkdir(parents=True, exist_ok=True)
    xml_003 = out_dir / '003_RESULT.xml'
    xml_004 = out_dir / '004_RESULT.xml'
    result_xml = out_dir / 'RESULT.xml'
    log_001 = logs_dir / '001_universal_pou_gvl.log'
    log_004 = logs_dir / '004_import_dut_v11.log'
    return compiler_dir, project_root, base_xml, xml_003, xml_004, result_xml, log_001, log_004
...
def extract_sections(st_text):
    sections = []
    remaining = st_text
    std = [('VAR_INPUT', 'inputVars'), ('VAR_OUTPUT', 'outputVars'), ('VAR_IN_OUT', 'inOutVars')]
    for keyword, xml_tag in std:
        ...
        while True:
            ...
            if not end_m:
                sections.append({'kind': keyword, 'xml_tag': xml_tag, 'body': None, 'mode': None, 'error': f'Missing END_VAR for {keyword}'})
                break
            body = remaining[m.end():m.end() + end_m.start()]
            sections.append({'kind': keyword, 'xml_tag': xml_tag, 'body': body, 'mode': None, 'error': None})
            remaining = remaining[:m.start()] + remaining[m.end() + end_m.end():]
    local_pattern = re.compile(r'(?im)^\s*VAR(?:\s+(CONSTANT|RETAIN))?\b')
    while True:
        m = local_pattern.search(remaining)
        if not m:
            break
        mode = m.group(1).upper() if m.group(1) else None
        end_m = re.search(r'(?im)^\s*END_VAR\b', remaining[m.end():])
        if not end_m:
            sections.append({'kind': 'VAR', 'xml_tag': 'localVars', 'body': None, 'mode': mode, 'error': 'Missing END_VAR for VAR'})
            break
        body = remaining[m.end():m.end() + end_m.start()]
        sections.append({'kind': 'VAR', 'xml_tag': 'localVars', 'body': body, 'mode': mode, 'error': None})
        remaining = remaining[:m.start()] + remaining[m.end() + end_m.end():]
...
def build_vars_xml(tag_name, variables, mode=None):
    if not variables:
        return ''
    attrs = ''
    if mode == 'CONSTANT':
...
def parse_decl(line):
    ...
    while i < len(rest) - 1:
        ...
        i += 1
    if val_pos >= 0:
        typ = rest[:val_pos].strip(); val = rest[val_pos+2:].strip()
    else:
        typ = rest.strip(); val = None
    ...
    for i, ch in enumerate(inner):
        if ch == "'" and (i == 0 or inner[i-1] != "\\"):
            ...
            if ch == '[': depth_sq += 1
                ...
                items.append(buf.strip()); buf = ''; continue
        buf += ch
    if buf.strip():
...
def parse_gvl_file(txt):
    ...
    for raw in txt.splitlines():
        ...
        if l.upper().startswith('VAR_GLOBAL'):
            up = l.upper(); mode = 'plain'
            if 'CONSTANT' in up: mode = 'constant'
            elif 'RETAIN' in up: mode = 'retain'
            cur = {'mode': mode, 'vars': []}; sections.append(cur); continue
        if l.upper().startswith('END_VAR'):
            cur = None; continue
        if cur is not None:
            v = parse_decl(l)
            if v: cur['vars'].append(v)
    return sections, qualified


def mode_attrs(mode):
    attrs = ''
    if mode == 'constant': attrs += ' constant="true"'
    if mode == 'retain': attrs += ' retain="true"'
    return attrs
...
def build_gvl_entry(name, sections, qualified):
    ...
    attr_block = ''
    if qualified:
        attr_block = '<data name="http://www.3s-software.com/plcopenxml/attributes" handleUnknown="implementation"><Attributes><Attribute Name="qualified_only" Value="" /></Attributes></data>'
    if len(non_empty) == 1:
        sec = non_empty[0]
        outer = build_section_globalvars_xml(name, sec['mode'], sec['vars'])
        outer = outer.replace('</globalVars>', '<addData>' + attr_block + '<data name="http://www.3s-software.com/plcopenxml/objectid" handleUnknown="discard"><ObjectId>' + object_id + '</ObjectId></data></addData></globalVars>')
    else:
        ...
        outer = outer.replace('</globalVars>', '<addData>' + attr_block + '<data name="http://www.3s-software.com/plcopenxml/objectid" handleUnknown="discard"><ObjectId>' + object_id + '</ObjectId></data><data name="http://www.3s-software.com/plcopenxml/mixedattrsvarlist" handleUnknown="implementation"><MixedAttrsVarList>' + mixed_inner + '</MixedAttrsVarList></data></addData></globalVars>')
    gvl = '<data name="http://www.3s-software.com/plcopenxml/globalvars" handleUnknown="implementation">' + outer + '</data>'
...
def build_003(project_root: Path, base_xml: Path, out_xml: Path, log_file: Path):
    ...
    if main_file.exists(): st_files.append(main_file)
    ...
    for f in st_files:
        ...
        for section in sections:
            if section['error']:
                local_warns.append(section['error']); continue
            variables, bad_lines = parse_var_block(section['body'])
            if bad_lines:
                local_warns.append(f"{section['kind']}: unparsed lines={len(bad_lines)}")
            block_xml = build_vars_xml(section['xml_tag'], variables, section.get('mode'))
            if block_xml: interface_parts.append(block_xml)
        if local_warns:
            pou_warn += 1
            logs.append(f.name + ': ' + '; '.join(local_warns))
        ...
        if target == 'pous': pous_tree.append((xml_name, object_id))
        else: prg_tree.append((xml_name, object_id))
    gvl_blocks, gvl_tree = [], []
    for f in list(project_root.glob('GVL*.gvl')):
        ...
        if entry == (None, None): continue
        g, obj = entry; gvl_blocks.append(g); gvl_tree.append(obj)
    ...
    gp = re.compile(r'(<addData>\s*)(<data name="http://www\.3s-software\.com/plcopenxml/globalvars" handleUnknown="implementation">.*?</data>\s*)+(?=<data name="http://www\.3s-software\.com/plcopenxml/projectstructure")', re.DOTALL)
    xml, n_gvl = gp.subn(lambda m: m.group(1) + '\n' + '\n'.join(gvl_blocks) + '\n', xml)
    if n_gvl != 1: raise RuntimeError(f'GVL block replace failed: {n_gvl}')
    xml = repl_folder(xml, 'GVL', gvl_tree)
    out_xml.write_text(xml, encoding='utf-8')
    logs.extend([f'POU_BLOCKS={len(pou_blocks)}', f'PRG_OBJECTS={len(prg_tree)}', f'GVL_BLOCKS={len(gvl_blocks)}', f'POU_WARNINGS={pou_warn}', 'DONE'])
...
def parse_enum(block: str):
    ...
    values_raw = m.group(1).strip(); base_type = (m.group(2) or 'INT').strip()
    parts = [p.strip() for p in values_raw.split(',') if p.strip()]
    values = []; auto = 0
    for part in parts:
        mm = re.match(r'^\s*([A-Za-z_][A-Za-z0-9_]*)\s*(?::=\s*([^\s,]+))?\s*$', part)
        if not mm: return None
        vname = mm.group(1); explicit = mm.group(2)
        if explicit is not None:
            values.append((vname, explicit))
            try: auto = int(explicit, 0) + 1
            except Exception: auto += 1
        else:
            values.append((vname, str(auto))); auto += 1
...
def parse_struct_fields(body: str):
    ...
    for ln in lines:
        if not ln: continue
        if ln.endswith(';'): ln = ln[:-1].strip()
        m = re.match(r'^([A-Za-z_][A-Za-z0-9_]*)\s*:\s*(.+)$', ln)
        if not m: continue
        fname = m.group(1).strip(); rest = m.group(2).strip(); finit = None
        if ':=' in rest:
            ftype, finit = rest.split(':=', 1); ftype = ftype.strip(); finit = finit.strip() or None
        else:
            ftype = rest.strip()
...
def build_type_ref(type_name: str) -> str:
    ...
    if tn.upper() in simple: return f'<{tn.upper()} />'
    m = re.match(r'(?is)^STRING\s*\[\s*([0-9]+)\s*\]$', tn)
    if m: return f'<string length="{m.group(1)}" />'
    m = re.match(r'(?is)^WSTRING\s*\[\s*([0-9]+)\s*\]$', tn)
    if m: return f'<wstring length="{m.group(1)}" />'
    m = re.match(r'(?is)^ARRAY\s*\[\s*([0-9]+)\s*\.\.\s*([0-9]+)\s*\]\s+OF\s+(.+)$', tn)
    if m:
        lo, hi, inner = m.group(1), m.group(2), m.group(3).strip(); inner_xml = build_type_ref(inner)
...
def is_simple_scalar(value: str) -> bool:
    ...
    if re.fullmatch(r'TRUE|FALSE', v, flags=re.IGNORECASE): return True
    if re.fullmatch(r'[+-]?\d+', v): return True
    if re.fullmatch(r'[+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?', v): return True
    if re.fullmatch(r"'[^']*'", v): return True
    if re.fullmatch(r'[A-Za-z_][A-Za-z0-9_\.]*', v): return True
    if re.fullmatch(r'(?:T|TOD|DT|DATE)#[^\s]+', v, flags=re.IGNORECASE): return True
    return False
...
def split_top_level_csv(text: str):
    ...
    while i < len(text):
        ...
        if ch == "'":
            buf.append(ch)
            if in_string:
                if i + 1 < len(text) and text[i + 1] == "'": buf.append(text[i + 1]); i += 1
...
def render_initializer_xml(finit: str):
    ...
    if v.startswith('[') and v.endswith(']'):
        inner = v[1:-1].strip(); values = [] if not inner else split_top_level_csv(inner)
        value_nodes = []
        for part in values:
...
def build_struct_xml(item):
    ...
    for fname, ftype, finit in item['fields']:
        tref = build_type_ref(ftype); init_xml = ''
        if finit is None:
            ...
            if kind in ('scalar', 'array'):
                with_initializer += 1
                if kind == 'array': array_initializer += 1
            else:
                without_initializer += 1; unsupported_initializer += 1
...
def find_matching_folder_end(text: str, folder_start: int) -> int:
    pos = folder_start; depth = 0
    while True:
        next_open = text.find('<Folder Name="', pos)
        next_close = text.find('</Folder>', pos)
        if next_close < 0: raise RuntimeError('Unbalanced Folder tags')
        if next_open != -1 and next_open < next_close:
            depth += 1; pos = next_open + 1; continue
        depth -= 1; pos = next_close + len('</Folder>')
        if depth == 0: return pos


def replace_dut_folder(xml: str) -> str:
    marker = '<Folder Name="DUT">'; start = xml.find(marker)
    if start < 0: raise RuntimeError('DUT folder not found in ProjectStructure')
...
def remove_old_dut_datatypes(xml: str):
    ...
    if not m: raise RuntimeError('dataTypes container not found')
    body = m.group(2); removed = 0
    def repl(mm):
        nonlocal removed
        name = mm.group(1)
        if name.startswith(TYPE_PREFIXES): removed += 1; return ''
        return mm.group(0)
...
def insert_before_closing(xml: str, closing_tag: str, insert_text: str) -> str:
    ...
    if idx < 0: raise RuntimeError(f'Closing tag not found: {closing_tag}')
    return xml[:idx] + insert_text + '\n' + xml[idx:]


def insert_into_child_folder(xml: str, parent_folder: str, child_folder: str, append_text: str) -> str:
    parent_marker = f'<Folder Name="{parent_folder}">'; child_marker = f'<Folder Name="{child_folder}">'
    parent_start = xml.find(parent_marker)
    if parent_start < 0: raise RuntimeError(f'Parent folder {parent_folder} not found')
    parent_end = find_matching_folder_end(xml, parent_start)
    child_start = xml.find(child_marker, parent_start, parent_end)
    if child_start < 0: raise RuntimeError(f'Child folder {child_folder} not found inside {parent_folder}')
    child_end = find_matching_folder_end(xml, child_start)
    insert_at = xml.rfind('</Folder>', child_start, child_end)
    if insert_at < 0: raise RuntimeError(f'Child folder closing tag not found for {child_folder}')
...
def build_004(project_root: Path, xml_003: Path, out_xml: Path, log_file: Path):
    if not xml_003.exists(): raise FileNotFoundError(f'003 xml not found: {xml_003}')
    xml = xml_003.read_text(encoding='utf-8', errors='replace')
    dut_files = sorted(project_root.glob('*.dut'))
    if not dut_files: raise RuntimeError('NO_DUT_FILES_FOUND')
    ...
    for f in dut_files:
        ...
        if not blocks:
            logs.append(f'{f.name}: no TYPE block'); continue
        for block in blocks:
            ...
            if item:
                ...
                logs.append(f"{f.name}: STRUCT {item['name']} fields={len(item['fields'])}"); continue
    ...
    for item in enum_types:
        x, o = build_enum_xml(item); enum_xml_blocks.append(x); enum_objs.append(o)
    ...
    for item in struct_types:
        ...
        initializers_inserted += inserted_count; array_initializers_inserted += array_count; unsupported_initializers_total += unsupported_count
    all_blocks = enum_xml_blocks + struct_xml_blocks
    xml = insert_before_closing(xml, '</dataTypes>', '\n'.join(all_blocks))
    if enum_objs: xml = insert_into_child_folder(xml, 'DUT', 'ENUM', '\n'.join(enum_objs))
    if struct_objs: xml = insert_into_child_folder(xml, 'DUT', 'STRUCT', '\n'.join(struct_objs))
    out_xml.write_text(xml, encoding='utf-8')
    xml_initial_value_count = xml.count('<initialValue>'); xml_array_value_count = xml.count('<arrayValue>')
    summary = [f'DUT_FILES={len(dut_files)}', f'REMOVED_OLD_DUT_DATATYPES={removed}', f'ENUM_INSERTED={len(enum_types)}', f'STRUCT_INSERTED={len(struct_types)}', f'STRUCT_FIELDS_TOTAL={struct_fields_total}', f'STRUCT_FIELDS_WITH_INITIALIZER={struct_fields_with_initializer}', f'INITIALIZERS_INSERTED={initializers_inserted}', f'ARRAY_INITIALIZERS_INSERTED={array_initializers_inserted}', f'UNSUPPORTED_INITIALIZERS={unsupported_initializers_total}', f'XML_INITIALVALUE_TAGS={xml_initial_value_count}', f'XML_ARRAYVALUE_TAGS={xml_array_value_count}', '']
    summary.extend(logs); summary.append(''); summary.extend(struct_summaries)
...
def main():
    ...
    compiler_dir, project_root, base_xml, xml_003, xml_004, result_xml, log_001, log_004 = script_layout(Path(__file__), args.xml, args.out, args.logs)
    ...
    try:
        ...
        overlay_script = _Path(__file__).parent / "method_overlay_clean.py"
        if overlay_script.exists():
            print("RUN METHOD OVERLAY...")
            rc = subprocess.call(["python3", str(overlay_script)])
            if rc != 0:
                raise RuntimeError("method_overlay_clean.py failed")
            print("METHOD OVERLAY DONE")

            # --- AUTO CLEANUP INTERMEDIATE FILES ---
            try:
                out_dir = _Path(__file__).parent / "out"
                for name in ("003_RESULT.xml", "004_RESULT.xml", "RESULT.xml"):
                    target = out_dir / name
                    if target.exists():
                        target.unlink()
