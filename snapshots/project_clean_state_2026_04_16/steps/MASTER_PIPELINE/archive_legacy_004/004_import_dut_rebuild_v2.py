...
LOG = Path("steps/MASTER_PIPELINE/004_import_dut.log")
ROOT = Path(".")
...
def build_struct_xml(item):
    oid = new_guid()
    vars_xml = []
    for fname, ftype in item['fields']:
        tref = build_type_ref(ftype)
        vars_xml.append(f'''            <variable name="{esc(fname)}">
              <type>
                {tref}
              </type>
            </variable>''')
